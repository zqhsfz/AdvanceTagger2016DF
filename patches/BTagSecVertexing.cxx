...
namespace Analysis {
  ...
  StatusCode BTagSecVertexing::BTagSecVtx_exec(xAOD::Jet& myJet, xAOD::BTagging* newBTag, xAOD::VertexContainer* bTagVertexContainer, xAOD::BTagVertexContainer* bTagJFVertexContainer, const xAOD::Vertex* vtx) {
	...
	if(sc.isFailure()){
  	...
  	sc = fillJFVariables(myJet, newBTag, bTagVertexContainer, bTagJFVertexContainer, myVertexInfoJetFitter, theTrackParticleContainer, basename);
  	if(sc.isFailure()){
  ...
  StatusCode BTagSecVertexing::fillJFVariables(xAOD::Jet& myJet,
     ...
     // convert TwoTrkVertex to xAOD::Vertex, and store to btagging vertex container
     std::vector<ElementLink<xAOD::VertexContainer> > v_el_xAODTwoTrkVtx;
     for(auto twoTrkVtx : vecTwoTrkVtx){
      auto recVertex = twoTrkVtx->recVertex();

      xAOD::Vertex* xAODTwoTrkVtx = new xAOD::Vertex();
      bTagVertexContainer->push_back(xAODTwoTrkVtx);       // setup aux store

      // convert VxCandidate to xAOD::Vertex
      // reference: http://acode-browser.usatlas.bnl.gov/lxr/source/atlas/Tracking/TrkVertexFitter/TrkVertexFitters/src/SequentialVertexFitter.cxx
      xAODTwoTrkVtx->setPosition( recVertex.position() );
      xAODTwoTrkVtx->setCovariancePosition( recVertex.covariancePosition() );
      xAODTwoTrkVtx->setFitQuality( recVertex.fitQuality().chiSquared(), recVertex.fitQuality().numberDoF() );
      xAODTwoTrkVtx->setVertexType( xAOD::VxType::NotSpecified );

      // the VxTrackAtVertex is empty this moment, since they are not used anyway
      std::vector<Trk::VxTrackAtVertex> vxTrackAtVertex;
      xAODTwoTrkVtx->vxTrackAtVertex() = vxTrackAtVertex;

      // association to tracks
      std::vector<ElementLink<xAOD::TrackParticleContainer> > v_el_track;
      for(auto vxTrk : *(twoTrkVtx->vxTrackAtVertex())){
        auto linkTrk = dynamic_cast<const Trk::LinkToXAODTrackParticle*>(vxTrk->trackOrParticleLink());
        auto trk = (**linkTrk);

        const xAOD::TrackParticleContainer* trkContainer = dynamic_cast<const xAOD::TrackParticleContainer*>(trk->container());
        ElementLink<xAOD::TrackParticleContainer> el_track(*trkContainer, trk->index());

        v_el_track.push_back(el_track);
      }
      xAODTwoTrkVtx->setTrackParticleLinks(v_el_track);

      // link from BTagging to two-track vertices
      const xAOD::VertexContainer* xAODTwoTrkVtxContainer = dynamic_cast<const xAOD::VertexContainer*>(xAODTwoTrkVtx->container());
      ElementLink<xAOD::VertexContainer> el_xAODTwoTrkVtx(*xAODTwoTrkVtxContainer, xAODTwoTrkVtx->index());
      v_el_xAODTwoTrkVtx.push_back(el_xAODTwoTrkVtx);
     }
     newBTag->setVariable(basename, "TwoTrackVerticesInJet", v_el_xAODTwoTrkVtx);
