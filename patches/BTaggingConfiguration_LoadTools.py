...
def SetupJetCollectionKmeans(JetCollection, TaggerList, ConfInstance = None):
  """Sets up the default B-Tagging configuration for jet collection called JetCollection. Returns True if successful. The function does nothing
  (and returns True) if a configuration for the given jet collection already exists; therefore the function is idempotent.
  ...
  # Setup associators
  BTagTrackToJetAssociatorKmeans = ConfInstance.setupTrackAssociator('BTagTrackToJetAssociatorKmeans', JetCollection, ToolSvc, Verbose = BTaggingFlags.OutputLevel < 3,
                                   options = {'existLabel': 'Kmeans_AssocTracks'})
  if 'SoftMu' in TaggerList or 'SoftMuChi2' in TaggerList:
    BTagMuonToJetAssociator = ConfInstance.setupMuonAssociator('Muons', JetCollection, ToolSvc, Verbose = BTaggingFlags.OutputLevel < 3)
  else:
    BTagMuonToJetAssociator = None
  if 'SoftEl' in TaggerList:
    ...
    BTagElectronToJetAssociator = None
  if 'MultiSVbb1' in TaggerList or 'MultiSVbb2' in TaggerList:
    ConfInstance.setupTrackAssociator('BTagTrackToJetAssociatorKmeansBB', JetCollection, ToolSvc,
                                    Verbose = BTaggingFlags.OutputLevel < 3,
                                    options={'existLabel': 'Kmeans_AssocTracks'})
...
#          if BTaggingFlags.IP1D:
#            addTool('IP1DTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'IP2D' in TaggerList:
    ConfInstance.addTool('IP2DTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DFlip:
#            addTool('IP2DFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DPos:
#            addTool('IP2DPosTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'IP2DNeg' in TaggerList:
    ConfInstance.addTool('IP2DNegTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DSpc:
#            addTool('IP2DSpcTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DSpcFlip:
#            addTool('IP2DSpcFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DSpcPos:
#            addTool('IP2DSpcPosTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP2DSpcNeg:
#            addTool('IP2DSpcNegTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'IP3D' in TaggerList:
    ConfInstance.addTool('IP3DTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'RNNIP' in TaggerList:
    ConfInstance.addTool('RNNIPTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel)

#          if BTaggingFlags.IP3DFlip:
#            addTool('IP3DFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP3DPos:
#            addTool('IP3DPosTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'IP3DNeg' in TaggerList:
    ConfInstance.addTool('IP3DNegTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP3DSpc:
#            addTool('IP3DSpcTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP3DSpcFlip:
#            addTool('IP3DSpcFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP3DSpcPos:
#            addTool('IP3DSpcPosTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.IP3DSpcNeg:
#            addTool('IP3DSpcNegTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  if 'SV1' in TaggerList:
    ConfInstance.addTool('SV1Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'SV2' in TaggerList:
    ConfInstance.addTool('SV2Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'SV1Flip' in TaggerList:
    ConfInstance.addTool('SV1FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.SV2:
#            addTool('SV2Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.SV2Flip:
#            addTool('SV2FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'SV0' in TaggerList:
    ConfInstance.addTool('SV0Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  if 'MultiSVbb1' in TaggerList:
    ConfInstance.addTool('MultiSVbb1Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeansBB', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'MultiSVbb2' in TaggerList:
    ConfInstance.addTool('MultiSVbb2Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeansBB', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.MultiSV:
#            # The name of this tagger was misspelled in the old code; I fixed it (Wouter)
#            addTool('MultiSVTag', ToolSvc, 'BTagTrackToJetAssociatorKmeansBB', JetCollection, Verbose =  BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.JetProb:
#            addTool('JetProbTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#            if BTaggingFlags.JetProbFlip:
#              addTool('JetProbFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.SoftEl:
#            addTool('SoftElectronTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'SoftMu' in TaggerList:
    ConfInstance.addTool('SoftMuonTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.SoftMuChi2:
#            addTool('SoftMuonTagChi2', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.BasicJetFitter:
#            addTool('JetFitterTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#            if BTaggingFlags.IP3D:
#              addTool('JetFitterCOMB', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.JetFitterTag:
#            addTool('NewJetFitterTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#            if BTaggingFlags.IP3D:
#              addTool('NewJetFitterCOMB', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'JetFitterNN' in TaggerList:
    ConfInstance.addTool('JetFitterTagNN', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    if 'IP3D' in TaggerList:
      ConfInstance.addTool('JetFitterTagCOMBNN', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.JetFitterTagFlip:
#            addTool('NewJetFitterTagFlip', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#            if BTaggingFlags.IP3DNeg:
#              addTool('NewJetFitterFlipCOMB', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'JetFitterNNFlip' in TaggerList:
    ConfInstance.addTool('JetFitterTagNNFlip', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#            if BTaggingFlags.IP3DPos:
#              addTool('JetFitterTagCOMBNNIP3DPos', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    if 'IP3DNeg' in TaggerList:
      ConfInstance.addTool('JetFitterTagCOMBNNIP3DNeg', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.TrackCounting:
#            addTool('TrackCounting', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.TrackCountingFlip:
#            addTool('TrackCountingFlip', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

#          if BTaggingFlags.GbbNNTag:
#            addTool('GbbNNTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  #MultivariateTagManager
  if (set(['DL1','MV2c00','MV2c10','MV2c20','MV2c100','MV2cl100','MV2m']) & set(TaggerList)):
    obj = ConfInstance.addTool('MultivariateTagManager', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if (set(['DL1Flip','MV2c00Flip','MV2c10Flip','MV2c20Flip','MV2c100Flip','MV2cl100Flip','MV2mFlip']) & set(TaggerList)):
    obj = ConfInstance.addTool('MultivariateFlipTagManager', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  if 'MV1' in TaggerList:
    ConfInstance.addTool('MV1Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'MV1c' in TaggerList:
    ConfInstance.addTool('MV1cTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'MV1cFlip' in TaggerList:
    ConfInstance.addTool('MV1cFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'MV1Flip' in TaggerList:
    ConfInstance.addTool('MV1FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#  if BTaggingFlags.MV2:
#    addTool('MV2Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  # Multivariate taggers
  if 'MVb' in TaggerList:
    tag = ConfInstance.addTool('MVbTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MVbFlip' in TaggerList:
    tag = ConfInstance.addTool('MVbFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c00' in TaggerList:
    tag = ConfInstance.addTool('MV2c00Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c00Flip' in TaggerList:
    tag = ConfInstance.addTool('MV2c00FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c10' in TaggerList:
    tag = ConfInstance.addTool('MV2c10Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c10Flip' in TaggerList:
    tag = ConfInstance.addTool('MV2c10FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c20' in TaggerList:
    tag = ConfInstance.addTool('MV2c20Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c20Flip' in TaggerList:
    tag = ConfInstance.addTool('MV2c20FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c100' in TaggerList:
    tag = ConfInstance.addTool('MV2c100Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c100Flip' in TaggerList:
    tag = ConfInstance.addTool('MV2c100FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2cl100' in TaggerList:
    tag = ConfInstance.addTool('MV2cl100Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2cl100Flip' in TaggerList:
    tag = ConfInstance.addTool('MV2cl100FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2m' in TaggerList:
    tag = ConfInstance.addTool('MV2mTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2mFlip' in TaggerList:
    tag = ConfInstance.addTool('MV2mFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c10hp' in TaggerList:
    tag = ConfInstance.addTool('MV2c10hpTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'MV2c10hpFlip' in TaggerList:
    tag = ConfInstance.addTool('MV2c10hpFlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'JetVertexCharge' in TaggerList:
    ConfInstance.addTool('JetVertexCharge', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)

  if 'DL1' in TaggerList:
    tag = ConfInstance.addTool('DL1Tag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'DL1Flip'in TaggerList:
    obj = ConfInstance.addTool('MultivariateFlipTagManager', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    tag = ConfInstance.addTool('DL1FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList += [tag,]
  if 'TagNtupleDumper' in TaggerList:
    tag = ConfInstance.addTool('TagNtupleDumper', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
    obj.MVTagToolList.append(tag)

  if 'ExKtbb_Hbb_MV2Only' in TaggerList:
    ConfInstance.addTool('ExKtbb_Hbb_MV2OnlyTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'ExKtbb_Hbb_MV2andJFDRSig' in TaggerList:
    ConfInstance.addTool('ExKtbb_Hbb_MV2andJFDRSigTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
  if 'ExKtbb_Hbb_MV2andTopos' in TaggerList:
    ConfInstance.addTool('ExKtbb_Hbb_MV2andToposTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
#          if BTaggingFlags.MV2Flip:
#            addTool('MV2FlipTag', ToolSvc, 'BTagTrackToJetAssociatorKmeans', JetCollection, Verbose = BTaggingFlags.OutputLevel < 3)
